...
def search_items(
    ...
    if query:
        try:
            ...
            result = sb.table('products').select('id, title, description, category, rating, avg_sentiment, review_count, tags').ilike('title', f'%{escaped_query}%').order('rating', desc=True).limit(limit).execute()
            products = result.data or []
            for p in products:
                p['rank'] = 0.0
    else:
        result = sb.table('products').select('id, title, description, category, rating, avg_sentiment, review_count, tags').order('rating', desc=True).order('review_count', desc=True).limit(limit).offset(offset).execute()
        products = result.data or []
